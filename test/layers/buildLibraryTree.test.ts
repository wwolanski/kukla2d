...
import {
  buildLibraryTree,
  flattenLibraryTree,
} from "@/features/layers/domain/buildLibraryTree";
...
describe("buildLibraryTree", () => {
  ...
  it("annotates protected modular sources and ordinary part assets", () => {
    ...
    expect(rows.find((row) => row.id === "source")).toMatchObject({
      modularKind: "source",
      modularSpriteId: "set-1",
    });
    expect(rows.find((row) => row.id === "head")).toMatchObject({
      modularKind: "part",
      partKey: "head",
    });
  });
  ...
  it("uses extension-free unique display names for legacy duplicate textures", () => {
    ...
    expect(rows.map((row) => row.sourceFileName)).toEqual([
      "Right Arm.png",
      "Right Arm.png",
      "Right Arm.png",
    ]);
  ...
  it("prefers persistent texture display name over fileName", () => {
    const rows = buildLibraryTree({
      textures: [
        texture("t1", { name: "Custom Name", fileName: "source.png" }),
      ],
      nodes: [],
    });
...
describe("flattenLibraryTree", () => {
  it("flattens nested folders into a flat list", () => {
    const tree: ReturnType<typeof buildLibraryTree> = [
      {
        kind: "folder",
        id: "f1",
        name: "Root",
