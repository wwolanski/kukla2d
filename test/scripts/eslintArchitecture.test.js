  ...
  it("fails a cross-feature internal import with an actionable ARCH-007 message", async () => {
    ...
    expect(
      ...
      expect.stringContaining(
        "ARCH-007: Cross-module deep import is forbidden.",
      ),
    );
  });
...
describe("ESLint domain purity", () => {
  const domainFixture =
    "src/features/layers/domain/__eslintArchitectureFixture.js";
  ...
  it("rejects presentation frameworks with ARCH-012", async () => {
    ...
    expect(
      ...
      expect.stringContaining(
        "ARCH-012: Domain must not depend on presentation frameworks.",
      ),
    );
  });
  ...
  it("rejects application state frameworks with ARCH-016", async () => {
    ...
    expect(
      ...
      expect.stringContaining(
        "ARCH-016: Domain must not depend on application state frameworks or stores.",
      ),
    );
  });

  it("rejects browser globals with ARCH-RS-1", async () => {
    const messages = await lintFixture(
      domainFixture,
      "export const browserDocument = document\n",
    );
    ...
    expect(
      ...
      expect.stringContaining(
        "ARCH-RS-1: Domain must not use browser runtime globals.",
      ),
    );
  });
