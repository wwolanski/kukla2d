...
} from "@/features/modular-sprite/application/finalizeModularSpriteImport.types";
import type {
  DetectedRegion,
  ProcessedModularSprite,
  RgbaImageData,
} from "@/features/modular-sprite/domain/contracts.types";
import type { RegionGrouping } from "@/features/modular-sprite/domain/partGrouping.types";
...
function processed(id: number): ProcessedModularSprite {
  return {
    ...
    observation: {
      ...
      processorVersion: 1,
      canvas: { width: 2, height: 2, aspectRatio: 1 },
      foregroundBounds: region(id).normalizedBounds,
      components: [],
      segmentationQualityBp: 0,
    },
  };
}
  ...
  parts: [
    {
      ...
      contentBounds: { x: 0, y: 0, width: 0.5, height: 0.5 },
      regionIds: [1],
    },
  ],
  excludedRegionIds: [],
...
describe("finalizeModularSpriteImport", () => {
  it("validates, reconciles full-resolution IDs, extracts and builds a commit request", async () => {
    ...
    const extractedImage: RgbaImageData = {
      width: 2,
      height: 2,
      data: new Uint8ClampedArray(16),
    };
    let extractedParts: ReadonlyArray<{
      partKey: string;
      regionIds: number[];
    }> = [];
  ...
  it("rejects an unconfirmed or empty part before invoking processing", async () => {
    ...
    const processing: ModularSpriteProcessingPort = {
      process: () => {
        processedCall = true;
        return Promise.resolve(processed(10));
      },
      extract: () => Promise.resolve([]),
    };
    await expect(
      finalizeModularSpriteImport(
        {
          source: { file: new File([], "hero.png"), image, preview: image },
          recipe: {
            background: {
        ...
        {
          processing,
          image: { encode: () => Promise.resolve(new Blob()) },
          schema: schemaPort,
        },
      ),
    ).rejects.toThrow("assigned region");
