...
describe("Pixi transform draft regressions", () => {
  it("moves from rendered animation pose instead of stale project transform", () => {
    ...
    const effectiveNodes = [
      { ...project.nodes[0], transform: { x: 100, y: 200 } },
    ];
    const { adapter, draft } = createAdapter({
      editor,
      project,
      effectiveNodes,
    });
  ...
  it("resizes in the rendered post-IK coordinate system", () => {
    ...
    const { adapter, preview } = createAdapter({
      editor,
      project,
      effectiveNodes,
    });
  ...
  it("resizes from every corner while keeping the opposite corner fixed", () => {
    ...
    for (let cornerIndex = 0; cornerIndex < frame.length; cornerIndex++) {
      ...
      startResizeDrag(adapter, { world: start }, cornerIndex, {
        bboxPoints: frame,
      });
      ...
      expect(preview.get("part").scaleX).toBeCloseTo(2);
      expect(preview.get("part").scaleY).toBeCloseTo(2);
      expect(preview.get("part").x).toBeCloseTo(
        expectedPosition[cornerIndex].x,
      );
      expect(preview.get("part").y).toBeCloseTo(
        expectedPosition[cornerIndex].y,
      );
    }
  });
  ...
  it("blocks bone translation in staging POSE", () => {
    ...
    const effectiveBones = [
      { ...project.bones[0], setup: { x: 50, y: 60, rotation: 0, length: 80 } },
    ];
    const { adapter, draft } = createAdapter({
      editor,
      project,
    ...
    expect(project.bones[0].setup).toEqual({
      x: 0,
      y: 0,
      rotation: 0,
      length: 80,
  ...
  it("rotates a bone in ANIMATION through authoring draft, not project setup", () => {
    ...
    const effectiveBones = [
      {
        ...project.bones[0],
        setup: { x: 10, y: 20, rotation: 30, length: 80 },
      },
    ];
    const { adapter, draft } = createAdapter({
      editor,
  ...
  it("blocks bone length drag in Animation mode (R5)", () => {
    ...
    const effectiveBones = [
      { ...project.bones[0], setup: { x: 0, y: 0, rotation: 90, length: 100 } },
    ];
    const { adapter, draft } = createAdapter({
      editor,
      project,
