import type {
  ...
  AssetPlacement,
  BoneSetup,
} from "../../packages/contracts/src/project.types.js";
import type {
  NodeId,
  BoneId,
  AssetId,
  ...
  SkinId,
  ConstraintId,
} from "../../packages/contracts/src/errors.types.js";
...
const mesh: Mesh = {
  ...
  influences: [
    ...
    [
      { boneId: B("b1"), weight: 0.8 },
      { boneId: B("b2"), weight: 0.2 },
    ],
    [{ boneId: B("b2"), weight: 1 }],
    [
      { boneId: B("b1"), weight: 0.5 },
      { boneId: B("b2"), weight: 0.5 },
    ],
  ],
};
...
const defaultPose: Record<
  string,
  Record<string, number | boolean | { x: number; y: number }[]>
> = {
...
const libraryFolders: LibraryFolder[] = [
  {
    ...
    name: "Character",
    parentId: null,
    sourceFileName: "char.psd",
    origin: "import",
  },
];

const assetPlacements: AssetPlacement[] = [{ assetId: "face", folderId: "f1" }];
