...
test("imports a synthetic modular sprite and reopens its protected source", async ({
  ...
  const firstRole = page.getByRole("combobox", { name: /Role:/ }).first();
  await firstRole.click();
  await page.getByRole("option", { name: "Head" }).click();
  const firstPartName = page.getByLabel("Name of part 1");
  await expect(firstPartName).toHaveValue("Head");
  await page
    .getByRole("button", { name: "Accept suggested name Head" })
    .click();

  await page.getByRole("button", { name: "Continue" }).click();
  await expect(
    page.getByTitle("Generated automatically from the part name").first(),
  ).toHaveValue("head");
  await expect(
  ...
  await expect(
    page.locator('#layers-panel span[title="synthetic-sheet"]'),
  ).toBeVisible();
