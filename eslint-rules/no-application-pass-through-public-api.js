...
function readModule(filePath) {
  ...
  if (cached?.mtimeMs === stat.mtimeMs && cached.size === stat.size) return cached.module
  ...
  parsedFileCache.set(filePath, { module, mtimeMs: stat.mtimeMs, size: stat.size })
  return module
}
...
function resolveModule(importerPath, specifier, cwd) {
  ...
  const basePath = extension ? unresolved.slice(0, -extension.length) : unresolved
  const candidates = extension
    ? [...SOURCE_EXTENSIONS.map(candidateExtension => `${basePath}${candidateExtension}`), unresolved]
    : [
        ...SOURCE_EXTENSIONS.map(candidateExtension => `${unresolved}${candidateExtension}`),
        ...SOURCE_EXTENSIONS.map(candidateExtension => path.join(unresolved, `index${candidateExtension}`)),
      ]
...
function createTracer({ cwd, featureRoot }) {
  ...
  const domainExportsName = (filePath, exportName, visited = new Set()) => {
    ...
    if (module.local.has(exportName) || module.explicit.has(exportName)) return true
  ...
  const traceNamed = (filePath, exportName, visited = new Set()) => {
    ...
    if (module.local.has(exportName) || explicitEntries.length > 1) {
      ...
      result = traceTarget(filePath, entry.importedName, entry.source, nextVisited)
    } else {
      const starResults = module.stars.map(specifier => {
        const target = resolveModule(filePath, specifier, cwd)
        if (!target) return { kind: 'absent' }
        if (isInside(domainRoot, target)) {
          return domainExportsName(target, exportName)
            ? { applicationFile: filePath, domainFile: target, domainSymbol: exportName, kind: 'domain' }
            : { kind: 'absent' }
        }
        if (!isInside(applicationRoot, target)) return { kind: 'other' }
        return traceNamed(target, exportName, nextVisited)
      }).filter(candidate => candidate.kind !== 'absent')
      result = starResults.length === 0
        ? { kind: 'absent' }
  ...
  const tracePureExportAll = (filePath, visited = new Set()) => {
    ...
    const module = readModule(filePath)
    if (!module || module.sourceFile.statements.length !== 1 || module.moduleStatements.length !== 1) {
      return null
    }
    ...
    if (isInside(domainRoot, target)) {
      return { applicationFile: filePath, domainFile: target, domainSymbol: '*' }
    }
...
function featureRootFor(filename) {
  ...
  if (!/^index\.(?:ts|tsx|js|jsx)$/.test(path.basename(normalized))) return null
  const featureRoot = path.dirname(normalized)
  const parentParts = path.dirname(featureRoot).split(path.sep)
  if (parentParts.at(-1) !== 'features' || parentParts.at(-2) !== 'src') return null
  return featureRoot
}
...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Disallow Domain symbols exposed through pass-through Application re-exports.',
    },
    schema: [],
    messages: {
      passThrough: 'ARCH-014: Domain symbol "{{symbol}}" is exposed through a pass-through Application re-export.\nReason: Application adds no behavior or orchestration to this public API symbol.\nApplication file: {{applicationFile}}\nDomain source: {{domainFile}}\nFix: Re-export "{{symbol}}" directly from Domain in the module root public API, or implement genuine Application-level behavior if this operation is intended to belong to Application.',
    },
  },
  ...
  create(context) {
    ...
    const report = (node, symbol, result) => {
      context.report({
        ...
        data: {
          applicationFile: relativeDisplayPath(featureRoot, result.applicationFile),
