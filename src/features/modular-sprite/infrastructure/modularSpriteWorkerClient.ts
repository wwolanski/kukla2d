...
export function createModularSpriteWorkerClient(
  options: ModularSpriteWorkerClientOptions = {},
): ModularSpriteWorkerClient {
    ...
    if (!worker) {
      ...
      next.onmessage = (event: MessageEvent<ModularSpriteWorkerResponse>) => {
        ...
        if (message.type === "error")
          entry.reject(new Error(message.data.message));
      ...
      next.onerror = (event) => {
        ...
        const error =
          event.error instanceof Error
            ? event.error
            : new Error(event.message || "Modular sprite worker failed");
  ...
  function cancelPending(): void {
    ...
    for (const [requestId, entry] of pending) {
      current?.postMessage({
        type: "abort",
        requestId,
      } satisfies ModularSpriteWorkerRequest);
      entry.reject(
        new DOMException("Modular sprite task cancelled", "AbortError"),
      );
    }
    pending.clear();
  }
  ...
  function dispatch<T>(
    ...
    if (disposed)
      return Promise.reject(
        new Error("Modular sprite worker client is disposed"),
      );
    cancelPending();
  ...
  function process(
    request: ModularSpriteProcessRequest,
