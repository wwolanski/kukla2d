...
export async function decodeModularSpriteFile(
  file: File,
): Promise<RgbaImageData> {
  if (file.size > MODULAR_SPRITE_MAX_FILE_BYTES)
    throw new Error("Image exceeds the 50 MiB limit");
  if (file.type && !SUPPORTED_TYPES.has(file.type))
    throw new Error("Use a static PNG, JPEG, or WebP image");
  const bitmap = await createImageBitmap(file);
  try {
    if (
      bitmap.width > MODULAR_SPRITE_MAX_SIDE ||
      bitmap.height > MODULAR_SPRITE_MAX_SIDE
    ) {
      throw new Error(
        `Image dimensions exceed ${MODULAR_SPRITE_MAX_SIDE} px per side`,
      );
    }
...
export function createPreviewImage(
  image: RgbaImageData,
  maximumSide = MODULAR_SPRITE_PREVIEW_SIDE,
): RgbaImageData {
  ...
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob(
      (blob) =>
        blob
          ? resolve(blob)
          : reject(new Error("PNG encoder returned no data")),
      "image/png",
    );
  });
}
