...
type ModularSpriteWorkerResult =
  ProcessedModularSprite | ExtractedPart[] | { warmed: true };

export type ModularSpriteWorkerResponse =
  | {
      type: "progress";
      data: { requestId: string; progress: number; stage: string };
    }
  | {
      type: "result";
      data: { requestId: string; result: ModularSpriteWorkerResult };
    }
  | {
      type: "error";
      data: {
        requestId: string;
        code:
          | "MODULAR_SPRITE_PROCESSING_FAILED"
          | "MODULAR_SPRITE_WARM_CACHE_MISSING";
        message: string;
      };
    };
