...
export function slotsFromModularSpriteParts(
...
export function groupingFromSchemaMatch(
  ...
  for (const [index, slot] of schema.slots.entries()) {
    const assignment = match.assignments.find(
      (item) => item.slotKey === slot.slotKey,
    );
    const regions = result.regions.filter((region) =>
      assignment?.componentIds.includes(region.id),
    );
    if (regions.length === 0) continue;
    const semantic = slot.semanticRoleId
      ? semantics?.get(slot.semanticRoleId)
      : undefined;
    const side = slot.qualifiers.side;
    const base = createPart(
      syntheticRegion(regions, result.width, result.height),
      index,
      parts,
    );
    parts.push({
  ...
  for (const region of result.regions) {
    ...
    parts.push({
      ...createPart(region, parts.length, parts),
      regionIds: [region.id],
    });
  }
...
export function portableModularSpriteSchema(
  schema: ModularSpriteSchema,
