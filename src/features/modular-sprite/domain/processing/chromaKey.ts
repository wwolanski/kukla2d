...
export function precomputeOklabRange(
  image: RgbaImageData,
  oklab: Float32Array,
  yStart: number,
  yEnd: number,
...
export async function precomputeOklabAsync(
  image: RgbaImageData,
...
export function computeMatteRange(
  ...
  for (let pixelIndex = startPixel; pixelIndex < endPixel; pixelIndex += 1) {
    ...
    const distance = Math.sqrt(
      deltaLightness * deltaLightness + deltaA * deltaA + deltaB * deltaB,
    );
    const keyAlpha = smoothstep(
      background.tolerance,
      background.tolerance + background.softness,
      distance,
    );
    ...
    if (keyAlpha > 0.02 && keyAlpha < 0.995 && background.despill > 0) {
      ...
      rgba[offset + 1] = Math.round(
        green + (correctedGreen - green) * edgeStrength,
      );
      rgba[offset + 2] = Math.round(
        blue + (correctedBlue - blue) * edgeStrength,
      );
    }
    rgba[offset + 3] = matte[pixelIndex] ?? 0;
  }
