...
function retainLargestComponent(
  heap: ComponentStats[],
  ...
  while (true) {
    ...
    if (
      right < heap.length &&
      hasLowerRetentionPriority(heap[right]!, heap[left]!)
    )
      lowest = right;
...
export function connectedComponents(
  ...
  const discardedRegionCount = Math.max(
    0,
    validRegionCount - components.length,
  );
  const valid = components.sort(
    (left, right) =>
...
export function restoreSplitPixels(
  labels: Int32Array,
  beforeSplit: Uint8Array,
  width: number,
  height: number,
): void {
