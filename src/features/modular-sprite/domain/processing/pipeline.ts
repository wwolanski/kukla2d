...
function buildRegions(
  ...
  const minimumArea = Math.max(
    1,
    Math.round(width * height * recipe.detection.minimumRegionAreaRatio),
  );
  const { labels, stats, discardedRegionCount } = connectedComponents(
    detection,
    width,
    height,
    minimumArea,
  ...
  const regions: DetectedRegion[] = stats.map((component) => {
    ...
    return {
      ...
      normalizedBounds,
      centroid,
      suggestedRole: suggestRole(
        normalizedBounds,
        component.area / (width * height),
      ),
      contour: componentContour(
        labels,
        component.id,
        width,
        height,
...
function warningsFor(
  ...
  if (discardedRegionCount > 0)
    warnings.push(
      `Showing the ${MAX_DETECTED_REGIONS} largest regions; ${discardedRegionCount} smaller regions were ignored. Increase the minimum region area to remove noise.`,
    );
  return warnings;
}
...
function makeResult(
  ...
  return {
    ...
    observation: createSpriteObservation({
      width: image.width,
      height: image.height,
      matte,
      labels,
...
export function processModularSprite(
...
export async function processModularSpriteAsync(
  request: ProcessModularSpriteRequest,
