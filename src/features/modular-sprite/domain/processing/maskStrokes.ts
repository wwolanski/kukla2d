...
export function rasterizeStroke(
  ...
  width: number,
  height: number,
  stroke: ModularSpriteMaskStroke,
  value: number,
): void {
  const config = MODULAR_SPRITE_PROCESSING_CONFIG.strokes;
  const radius = Math.max(
    config.minimumPixelRadius,
      ...
      const steps = Math.max(
        ...
        Math.ceil(
          distance /
            Math.max(
              config.minimumPixelRadius,
              radius * config.sampleSpacingRadiusFactor,
            ),
        ),
      );
...
export function matteStrokesPass(
  ...
  const alphaStrokes = recipe.strokes.filter(
    (stroke) => stroke.kind === "foreground" || stroke.kind === "background",
  );
  if (alphaStrokes.length === 0) return;
  for (const stroke of alphaStrokes) {
    if (stroke.kind === "foreground")
      rasterizeStroke(
        matte,
        width,
        height,
    ...
    if (stroke.kind === "background")
      rasterizeStroke(matte, width, height, stroke, 0);
  }
  for (let index = 0; index < matte.length; index += 1)
    rgba[index * 4 + 3] = matte[index] ?? 0;
}

export function detectionStrokesPass(
  detection: Uint8Array,
  recipe: ModularSpriteProcessingRecipe,
  width: number,
  height: number,
): Uint8Array | null {
  const hasSplitStroke = recipe.strokes.some(
    (stroke) => stroke.kind === "split",
  );
  const beforeSplit = hasSplitStroke ? new Uint8Array(detection) : null;
  for (const stroke of recipe.strokes) {
    if (stroke.kind === "split")
      rasterizeStroke(detection, width, height, stroke, 0);
    if (stroke.kind === "foreground")
      rasterizeStroke(detection, width, height, stroke, 1);
    if (stroke.kind === "background")
      rasterizeStroke(detection, width, height, stroke, 0);
  }
  return beforeSplit;
