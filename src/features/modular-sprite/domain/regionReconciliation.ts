...
function pairConfidence(
  previous: DetectedRegion,
  next: DetectedRegion,
): number {
  const iou = rectIntersectionOverUnion(
  ...
  const previousArea = Math.max(
    Number.EPSILON,
    previous.normalizedBounds.width * previous.normalizedBounds.height,
  );
  const nextArea = Math.max(
    Number.EPSILON,
    next.normalizedBounds.width * next.normalizedBounds.height,
  );
...
export function mapRegions(
  ...
  const scores = new Map<
    number,
    Array<{ region: DetectedRegion; score: number }>
  >();
  for (const previous of previousRegions) {
    const candidates = nextRegions
      ...
      .filter(
        (candidate) =>
          candidate.score >= candidateThreshold(previous, candidate.region),
      )
      .sort(
        (left, right) =>
          right.score - left.score || left.region.id - right.region.id,
      );
  ...
  for (const [previousId, candidates] of scores) {
    for (const candidate of candidates) {
      ...
      if (
        ...
        (candidate.score === current.score && previousId < current.previousId)
      ) {
        winners.set(candidate.region.id, {
          previousId,
  ...
  const mappings: RegionMapping[] = previousRegions.map((previous) => {
    const candidates = (scores.get(previous.id) ?? [])
      .filter(
        (candidate) =>
          winners.get(candidate.region.id)?.previousId === previous.id,
      )
      .filter(
        (candidate, index, all) =>
          index === 0 ||
          candidate.score >= Math.max(0.2, (all[0]?.score ?? 0) * 0.55),
      )
    ...
    const confidence =
      candidates.length > 0
        ? Math.max(
            ...(scores.get(previous.id) ?? [])
              .filter((candidate) => candidates.includes(candidate.region.id))
              .map((candidate) => candidate.score),
          )
        : 0;
    return {
      previousRegionId: previous.id,
      nextRegionIds: candidates,
      confidence,
    };
  });
  const mappedNext = new Set(
    mappings.flatMap((mapping) => mapping.nextRegionIds),
  );
  return {
    mappings,
    lostPreviousRegionIds: mappings
      .filter((mapping) => mapping.nextRegionIds.length === 0)
      .map((mapping) => mapping.previousRegionId),
    uncertainPreviousRegionIds: mappings
      .filter(
        (mapping) =>
          mapping.nextRegionIds.length > 1 ||
          (mapping.nextRegionIds.length > 0 && mapping.confidence < 0.55),
      )
      .map((mapping) => mapping.previousRegionId),
    unmatchedNextRegionIds: nextRegions
      .map((region) => region.id)
      .filter((id) => !mappedNext.has(id)),
  };
}
...
export function reconcileRegionGrouping(
  ...
  const mapping = new Map(
    report.mappings.map((item) => [item.previousRegionId, item.nextRegionIds]),
  );
  return {
    grouping: reconcileGrouping(
      grouping,
      mapping,
