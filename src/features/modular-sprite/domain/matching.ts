...
function matchConfidence(
  template: ModularSpriteTemplatePart,
  ...
  const centerScore =
    ...
    clamp(
      Math.hypot(
        expectedCenterX - actualCenterX,
        expectedCenterY - actualCenterY,
      ) / Math.SQRT2,
      0,
      1,
    );
  ...
  const aspectScore = Math.exp(
    -Math.abs(Math.log(actualAspect / expectedAspect)),
  );
  return clamp(
    centerScore * 0.35 +
      intersectionScore * 0.35 +
      areaScore * 0.2 +
      aspectScore * 0.1,
    0,
    1,
...
function hungarian(cost: readonly (readonly number[])[]): number[] {
  ...
  if (columnCount < rowCount)
    throw new Error(
      "Hungarian matrix must have at least as many columns as rows",
    );
...
export function matchRegionsToTemplate(
  ...
  const assignment = hungarian(
    confidences.map((row) => row.map((confidence) => 1 - confidence)),
  );
