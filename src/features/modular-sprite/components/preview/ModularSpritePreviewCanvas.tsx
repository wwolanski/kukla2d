...
function imageDataForMode(source: RgbaImageData, result: ProcessedModularSprite | null, mode: PreviewMode): ImageData {
  if (!result || mode === 'original') return new ImageData(new Uint8ClampedArray(source.data), source.width, source.height);
  if (mode === 'result') return new ImageData(new Uint8ClampedArray(result.rgba), result.width, result.height);
  ...
  useEffect(() => {
    ...
    for (const region of result.regions) {
      ...
      if (region.contour.length > 1) {
        ...
        context.stroke();
      } else if (isExcluded) context.fillRect(region.bounds.x, region.bounds.y, region.bounds.width, region.bounds.height);
      ...
      context.fillText(assignment ? `${region.id} · ${assignment.name}` : `${region.id} · Excluded`, region.bounds.x + 3, region.bounds.y + 14);
    }
