    ...
    <ScrollArea className="h-full min-h-0 min-w-0 rounded-lg border">
      <aside className="space-y-4 p-4">
        <p className="text-xs text-muted-foreground">
          Group detected regions into the images that should be imported as
          parts. Part names come from detection or schema matching; correct them
          directly in the tree when needed. Anatomical roles are reviewed in the
          next step. Shift-click selects several regions.
        </p>
        <UiButton
          className="w-full"
          size="sm"
        ...
        <label className="grid gap-1 text-xs text-muted-foreground">
          ...
          <select
            ...
            value={assignmentPartKey}
            onChange={(event) => onAssignmentPartKeyChange(event.target.value)}
          >
            <option value="">Choose an existing part…</option>
            {grouping.parts.map((part) => (
              <option key={part.partKey} value={part.partKey}>
                {part.name}
              </option>
            ))}
          </select>
        ...
        <UiButton
          ...
          size="sm"
          variant="outline"
          disabled={!assignmentPartKey || selectedRegionIds.size === 0}
          onClick={onAssign}
        >
          Move selection to target part
        </UiButton>
        <UiButton
          className="w-full"
          size="sm"
        ...
        >
          ...
          Create new part from selection
        </UiButton>
        <UiButton
          className="w-full"
          size="sm"
          ...
          onClick={onExclude}
        >
          Exclude selection from import
        </UiButton>
        <p className="text-[11px] text-muted-foreground">
          Excluded regions stay visible in muted gray so you can select them and
          move them back into a part. They will not be imported.
        </p>
