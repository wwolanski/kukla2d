    ...
    <>
      <div className="border-b px-6 py-4">
        ...
        <p className="text-sm text-muted-foreground">
          Extract reusable transparent parts from an alpha or controlled
          chroma-key sheet.
        </p>
      </div>
      <div className="flex border-b px-6 py-2">
        {STEPS.map((item, index) => (
          <div
            key={item}
            className={`flex-1 text-center text-xs font-medium capitalize ${item === step ? "text-primary" : "text-muted-foreground"}`}
          >
