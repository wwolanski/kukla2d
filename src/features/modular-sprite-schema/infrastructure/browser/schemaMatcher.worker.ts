...
scope.onmessage = (event: MessageEvent<SchemaMatcherWorkerRequest>) => {
  ...
  if (message.type === "catalog") {
    ...
    scope.postMessage({
      type: "ready",
      catalogRevision,
    } satisfies SchemaMatcherWorkerResponse);
    return;
  }
  if (message.type === "abort") {
    aborted.add(message.requestId);
    return;
  }
  ...
  void (async () => {
    try {
      ...
      const service = new SchemaComparisonService(schemas, catalogRevision);
      const response = await service.matchAsync(request, {
        throwIfAborted: () => {
      ...
      aborted.delete(request.requestId);
    }
  })();
};
