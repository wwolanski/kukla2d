...
function collectPoseClipboardEntriesAtTime(
  ...
  for (const track of animation.tracks) {
    ...
    const keyframe = track.keyframes.find(
      (candidate) => candidate.time === timeMs,
    );
...
function hasAuthoredPoseChannel(
  entries: readonly PoseClipboardEntry[],
): boolean {
  return entries.some((entry) =>
    Object.values(entry.channels).some(
      (channel) => channel.role === "authored",
    ),
  );
}
...
function useKeyframeActionsImpl({
  ...
  const [poseClipboard, setPoseClipboard] = useState<PoseClipboard | null>(
    null,
  );
  ...
  const copyKeyframe = useCallback(
    (nodeId: AnimationTargetId, timeMs: number) => {
      if (!animation) return;
  ...
  const pasteKeyframes = useCallback(() => {
    if (!clipboard || !animation || !activeAnimationId || sel.length === 0)
      return;
  ...
  const copyPose = useCallback(() => {
    ...
    const entries = collectPoseClipboardEntriesAtTime(
      animation,
      playbackRef.current.currentTimeMs,
    );
  ...
  const pastePose = useCallback(
    (mirror = false) => {
      if (!poseClipboard || !animation || !activeAnimationId)
        return { changed: false };
      ...
      return {
        changed: keyframes.length > 0,
        sourceFrame: poseClipboard.sourceFrame,
        targetFrame: frame,
      };
    },
  ...
  const setEasingAt = useCallback(
    (
      ...
      if (optionalEasing === undefined) {
        ...
        properties =
          typeof propertiesOrTime === "number" ? null : propertiesOrTime;
        timeMs = typeof timeOrEasing === "number" ? timeOrEasing : 0;
        easingType = optionalEasing;
      }
      const addressesAtTime = getAddressesAtTime(
        animation,
        targetId,
        properties,
        timeMs,
      );
      const useSelection = addressesAtTime.some((address) =>
        selectedKeyframes.has(address),
      );
      const applyTo = useSelection
        ? selectedKeyframes
        : new Set(addressesAtTime);
      setKeyframeEasing({
        animationId: activeAnimationId,
...
export const useKeyframeActions = (
  ...args: Parameters<typeof useKeyframeActionsImpl>
): ReturnType<typeof useKeyframeActionsImpl> => useKeyframeActionsImpl(...args);
