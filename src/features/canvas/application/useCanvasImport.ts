...
interface CanvasImportController {
...
export function useCanvasImport(
  ...
  const handlePlaceLibraryAsset = useCallback(
    (assetId: string, event: CanvasDropEvent) =>
      placeLibraryAsset({
        assetId,
        event,
        projectRef,
  ...
  const importPsdFile = useCallback(
  ...
  const importStretchFile = useCallback(
    async (file: File): Promise<void> => {
