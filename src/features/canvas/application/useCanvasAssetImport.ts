...
export function useCanvasAssetImport({
  ...
  const importPng = useCallback(
    (file: File): Promise<void> =>
      new Promise((resolve, reject) => {
        const autoAddToCanvas =
          useImportSettingsStore.getState().autoAddToCanvas;
        const shouldCenterAfterImport =
          autoAddToCanvas && projectRef.current.nodes.length === 0;
        ...
        image.onload = () => {
          ...
          updateProject((projectDraft, versionControl) => {
            ...
            if (!projectDraft.assetPlacements)
            ...
            if (autoAddToCanvas) {
              projectDraft.nodes.push({
                ...
                name: displayName,
                parent: null,
                draw_order: projectDraft.nodes.filter(
                  (node) => node.type === "part",
                ).length,
                opacity: 1,
                visible: true,
  ...
  const finalizePsdImport = useCallback(
    (
      ...
      const autoAddToCanvas =
        !libraryOnly && useImportSettingsStore.getState().autoAddToCanvas;
      updateProject((projectDraft, versionControl) => {
        if (autoAddToCanvas) {
        ...
        const nodes: ImportedPartNode[] = layers.map((layer, index) => {
          ...
          if (!partId)
            throw new Error(`Missing generated part id for PSD layer ${index}`);
          const canvas = document.createElement("canvas");
          canvas.width = width;
          ...
          if (!context || !layerContext)
            throw new Error("Failed to create 2D context for PSD import");
          ...
          return {
            id: partId,
            type: "part",
            name: layerDisplayNames[index] ?? layer.name,
            parent: null,
            draw_order: layers.length - 1 - index,
        ...
        for (const node of applyHistoricalClipToPartId(nodes)) {
          projectDraft.textures.push({
            ...
            fileName:
              sourceFileNames.get(node.id) ?? `${node.name || node.id}.png`,
            fileSize: null,
          });
