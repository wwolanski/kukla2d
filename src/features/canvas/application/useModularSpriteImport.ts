  ...
  return useCallback(
    (
      ...
      const existing = request.existingId
        ? currentProject.modularSprites.find(
            (candidate) => candidate.id === request.existingId,
          )
        : undefined;
      if (request.existingId && !existing)
        throw new Error("The modular sprite no longer exists in this project");
      if (request.parts.length === 0)
        throw new Error("Assign at least one modular sprite part");
        ...
        updateProject((projectDraft, versionControl) => {
          ...
          const nextDocument = {
            ...
            parts: request.parts.map((part) => ({
              ...
              ...(part.draft.semanticRoleId
                ? { semanticRoleId: part.draft.semanticRoleId }
                : {}),
              ...(part.draft.qualifiers
                ? { qualifiers: structuredClone(part.draft.qualifiers) }
                : {}),
            ...
            ...(request.schemaBinding
              ? { schemaBinding: structuredClone(request.schemaBinding) }
              : {}),
          };
          const existingIndex = projectDraft.modularSprites.findIndex(
            (candidate) => candidate.id === modularSpriteId,
          );
          if (existingIndex >= 0)
            projectDraft.modularSprites[existingIndex] = nextDocument;
          ...
          if (!existing && request.addToCanvas) {
            ...
            const groupX =
              projectDraft.canvas.x +
              (projectDraft.canvas.width - request.sourceImage.width) / 2;
            const groupY =
              projectDraft.canvas.y +
              (projectDraft.canvas.height - request.sourceImage.height) / 2;
            ...
            const baseDrawOrder =
              Math.max(
                ...
                ...projectDraft.nodes
                  ...
                  .map((node) => node.draw_order),
              ) + 1;
            for (const part of [...request.parts].sort(
              (left, right) => left.draft.order - right.draft.order,
            )) {
              const nodeId = toNodeId(uid());
              createdNodeIds.push(nodeId);
              const frameX = Math.floor(
                part.draft.extractionFrame.x * request.sourceImage.width,
              );
              const frameY = Math.floor(
                part.draft.extractionFrame.y * request.sourceImage.height,
              );
              ...
              projectDraft.nodes.push({
                ...
                imageBounds: computeImageBounds(imageData) ?? {
                  minX: 0,
                  minY: 0,
                  maxX: part.image.width,
                  maxY: part.image.height,
                },
          ...
          for (const part of request.parts) {
            ...
            for (const node of projectDraft.nodes) {
              ...
              node.imageBounds = computeImageBounds(imageData) ?? {
                minX: 0,
                minY: 0,
                maxX: part.image.width,
                maxY: part.image.height,
      ...
      for (const url of stagedResources.transferOut())
      ...
      for (const part of request.parts) {
        ...
        const referencingNodes = useProjectStore
          .getState()
          .project.nodes.filter(
            (node) => node.type === "part" && node.textureId === assetId,
          );
      ...
      if (layoutGroupId)
        useEditorStore.getState().setSelection([layoutGroupId]);
      if (wasEmpty && request.addToCanvas)
        centerView(request.sourceImage.width, request.sourceImage.height);
      markDirty();
