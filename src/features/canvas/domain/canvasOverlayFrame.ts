...
interface OverlayFramePose extends Pick<
  FramePose,
  "poseOverrides" | "effectiveNodes" | "effectiveBones" | "effectiveMeshes"
> {
 ...
 */
function buildCanvasOverlayFrameImpl({
  project,
  editorState,
  animationState,
  framePose,
}: CanvasOverlayFrameInput): CanvasOverlayFrame {
  const pose =
    framePose ??
    buildFramePose({
      project,
  ...
  const worldMatrices =
    framePose?.worldMatrices ?? computeWorldMatrices(effectiveNodes);

  const selection = editorState?.selection ?? [];
  const activeTool =
    editorState?.activeTool ?? editorState?.toolMode ?? "select";
  const selectionTarget = editorState?.selectionTarget ?? "all";
  const showsElementChrome =
    ["select", "transform"].includes(activeTool) &&
  ...
  const hoverHit =
    editorState?.hoverSource === HOVER_SOURCE_PANEL
      ? visibleHoverHit
      : showsElementHover
        ? visibleHoverHit
        : null;
  ...
  const weightPaintPartId = editorState?.weightPaintMode
    ? (selection[0] ?? null)
    : null;
  const weightPaintBoneId =
    editorState?.weightPaintBoneId ?? editorState?.activeBoneId ?? null;
  ...
  const weightPaintPoints = weightPaintOverlay?.visible
    ? weightPaintOverlay.vertices.map((v, i) => ({
        x: v.x,
        y: v.y,
        weight: weightPaintOverlay.weights[i] ?? 0,
      }))
    : null;
  ...
  const exportAreaFrame = buildExportAreaOverlayFrame(
    project.canvas,
    showExportArea,
  );
...
export const buildCanvasOverlayFrame = (
  ...args: Parameters<typeof buildCanvasOverlayFrameImpl>
): CanvasOverlayFrame => buildCanvasOverlayFrameImpl(...args);
...
function applyConstraintPose(
  ...
  return {
    ...constraint,
    ...(typeof overrides.targetX === "number"
      ? { targetX: overrides.targetX }
      : {}),
    ...(typeof overrides.targetY === "number"
      ? { targetY: overrides.targetY }
      : {}),
    ...(typeof overrides.mix === "number" ? { mix: overrides.mix } : {}),
    ...(typeof overrides.fkIk === "number" ? { fkIk: overrides.fkIk } : {}),
    ...(typeof overrides.bendPositive === "boolean"
      ? { bendPositive: overrides.bendPositive }
      : {}),
  };
}

function buildIkOverlay({
  project,
  editorState,
  effectiveBones,
  poseOverrides,
  ...
  const constraints = (project.constraints ?? []).map((constraint) =>
    applyConstraintPose(constraint, poseOverrides?.get(constraint.id)),
  );
    ...
    return [
      {
        ...
        radius: getIkTargetRadius(
          constraint.assignedBoneId
            ? boneMap.get(constraint.assignedBoneId)
            : null,
        ),
  ...
  if (
    editorState.interaction?.kind === "pendingPickIKBone" ||
    ...
    const constraint = interaction?.constraintId
      ...
      : undefined;
    const hoverBoneId =
      interaction?.kind === "pendingSuggestIKBone"
        ? (interaction.boneId ?? null)
        : typeof editorState.hoverHit === "string" &&
  ...
  if (
    ...
    const bone = constraint?.assignedBoneId
      ? boneMap.get(constraint.assignedBoneId)
      : undefined;
    if (
      constraint &&
  ...
  if (
    ...
    const isActive =
      editorState.activeBoneId === boneId ||
    ...
    const constraint = !isActive
      ? constraints.find(
          (item) =>
            item.type === "ik" &&
            item.enabled !== false &&
            (item.assignedBoneId === boneId ||
              item.affectedBoneIds.some((id) => id === boneId)) &&
...
function buildMeshWireframe({
  ...
  const part = effectiveNodes.find(
    (node): node is PartNode => node.id === partId && node.type === "part",
  );
  ...
  const triangles = formatTriangles(
    effectiveFrame?.triangles?.length
...
type TriangleInput =
  readonly (readonly [number, number, number])[] | readonly number[];
...
function formatTriangles(
  triangles: TriangleInput | null | undefined,
): number[][] {
  if (!triangles || triangles.length === 0) return [];
  if (isTriangleTupleArray(triangles)) {
    return triangles.map((triangle) => [triangle[0], triangle[1], triangle[2]]);
  }
  return Array.from({ length: Math.floor(triangles.length / 3) }, (_, index) =>
    triangles.slice(index * 3, index * 3 + 3),
  );
}
...
function buildWeightPaintOverlayFrame({
  ...
  const part = effectiveNodes.find(
    (node): node is PartNode =>
      node.id === weightPaintPartId && node.type === "part",
  );
  ...
  const stats = computeMeshWeightStats(
    mesh,
    mesh.influences
      ?.flat()
      .find((influence) => influence.boneId === weightPaintBoneId)?.boneId ??
      null,
  );
  ...
  return {
    ...
    triangles,
    weights,
    selectedBoneId: weightPaintBoneId,
    stats,
  };
}
...
export function buildExportAreaOverlayFrame(
  ...
  const width =
    Number.isFinite(canvas.width) && canvas.width > 0 ? canvas.width : 0;
  const height =
    Number.isFinite(canvas.height) && canvas.height > 0 ? canvas.height : 0;
  return { x, y, width, height, valid: width > 0 && height > 0 };
}

function buildBrushCursor(
  editorState: CanvasOverlayEditorState | null | undefined,
