...
function captureLinkedAnimState(
  adapter: PixiInteractionSystem,
  projectNode: Node,
): Pick<
  Extract<DragState, { type: "move" }>,
  "linkedPreLinkedWorldMatrices" | "linkedBone"
> {
  ...
  const boneId =
    ...
    (part?.mesh?.influences ?? [])
      .flatMap((vertex) => vertex.map((influence) => influence?.boneId))
      .find(Boolean);
  return {
    linkedPreLinkedWorldMatrices: preLinkedNodes
      ? computeWorldMatrices(preLinkedNodes)
      : null,
    linkedBone:
      (framePose?.effectiveBones ?? []).find((bone) => bone.id === boneId) ??
      null,
  };
}
...
export function startMoveDrag(
  ...
  const node = getAdapterEffectiveRigState(adapter).nodes.find(
  ...
  const isLinked =
    isBoneLinkLocked(projectNode) &&
    !!getAssignedBoneForNode(project, projectNode.id);
  const isAnimMode = editorState.editorMode === "animation";
  const linkedAnim = isAnimMode && isLinked;
  if (!isAnimMode)
    adapter._beginCommandBatch({ name: "Transform drag", type: "transform" });
  ...
  adapter._setDragState({
    ...
    isAnimMode,
    isLinked: isAnimMode ? false : isLinked,
    linkedAnim,
  ...
  adapter._sendWorkflow({
    ...
    payload: { mode: "move", nodeId: node.id },
  });
}
...
export function startRotateDrag(
  ...
  const node = effectiveState.nodes.find(
    (candidate) => candidate.id === selection[0],
  );
  if (!projectNode || !node) return;
  const transform = node.transform ?? {};
  ...
  adapter._setDragState({
    ...
    pivotWorldY,
    isAnimMode,
    isLinked: !isAnimMode && isLinked,
    linkedAnim: isAnimMode && isLinked,
    ...(isAnimMode && isLinked
      ? captureLinkedAnimState(adapter, projectNode)
      : {}),
    lastDelta: 0,
    startAngle: Math.atan2(world.y - pivotWorldY, world.x - pivotWorldX),
    lastPatch: null,
    gestureId,
  });
  adapter._sendWorkflow({
    type: "START_TRANSFORM_DRAG",
    payload: { mode: "rotate", nodeId: node.id },
  });
}

export function startPivotDrag(
  adapter: PixiInteractionSystem,
  event: PointerInput,
): void {
  const editorState = adapter.editorRef.current;
  ...
  const matrix: Matrix3 = computeWorldMatrices(effectiveNodes).get(node.id) ?? [
    1, 0, 0, 0, 1, 0, 0, 0, 1,
  ];
  ...
  adapter._setDragState({
    ...
    startClientX: start.x,
    startClientY: start.y,
    startPivotX: transform.pivotX ?? 0,
    startPivotY: transform.pivotY ?? 0,
    startX: transform.x ?? 0,
    startY: transform.y ?? 0,
    iswm: mat3Inverse(matrix),
    lastPatch: null,
  });
  adapter._sendWorkflow({
    type: "START_TRANSFORM_DRAG",
    payload: { mode: "pivot", nodeId: node.id },
  });
}
...
export function startResizeDrag(
  ...
  const project = adapter.projectRef.current;
  if (!canStartAnimationGesture(adapter)) return;
  const projectNode = project.nodes.find(
    (node) => node.id === editorState.selection?.[0],
  );
  const effectiveState = getAdapterEffectiveRigState(adapter);
  const node = effectiveState.nodes.find(
    (candidate) => candidate.id === editorState.selection?.[0],
  );
  if (!projectNode || !node) return;
  const matrix = computeWorldMatrices(effectiveState.nodes).get(node.id);
  if (!matrix) return;
  ...
  const fixedCorner =
    frame.bboxPoints[(cornerIndex + 2) % frame.bboxPoints.length];
  if (!corner || !fixedCorner || frame.bboxPoints.length < 4) return;
  const cornerLocalX =
    inverseMatrix[0] * corner.x +
    inverseMatrix[3] * corner.y +
    inverseMatrix[6];
  const cornerLocalY =
    inverseMatrix[1] * corner.x +
    inverseMatrix[4] * corner.y +
    inverseMatrix[7];
  const fixedLocalX =
    inverseMatrix[0] * fixedCorner.x +
    inverseMatrix[3] * fixedCorner.y +
    inverseMatrix[6];
  const fixedLocalY =
    inverseMatrix[1] * fixedCorner.x +
    inverseMatrix[4] * fixedCorner.y +
    inverseMatrix[7];
  const transform = node.transform ?? {};
  const isAnimMode = editorState.editorMode === "animation";
  ...
  if (!isAnimMode)
    adapter._beginCommandBatch({ name: "Resize drag", type: "transform" });
  const gestureId = isAnimMode
    ? adapter.animationAuthoringAdapter?.beginGesture()
    : null;
  adapter._setDragState({
    ...
    nodeId: node.id,
    iswm: inverseMatrix,
    startWorldMatrix: new Float32Array(matrix),
    fixedWorldX: fixedCorner.x,
    fixedWorldY: fixedCorner.y,
    ...
    startY: transform.y ?? 0,
    startRotation: transform.rotation ?? 0,
    startScaleX: transform.scaleX ?? 1,
    startScaleY: transform.scaleY ?? 1,
    isAnimMode,
    isLinked: !isAnimMode && isLinked,
    linkedAnim: isAnimMode && isLinked,
    ...(isAnimMode && isLinked
      ...
      ? {
          linkedScaleSnapshot: {
            ...
            nodeTransforms: Object.fromEntries(
              getLinkedNodesForBone(project, assignedBone.id).map(
                (linkedNode) => [linkedNode.id, { ...linkedNode.transform }],
              ),
            ),
          },
        }
      : {}),
    lastScaleX: transform.scaleX ?? 1,
    lastScaleY: transform.scaleY ?? 1,
    lastPatch: null,
    gestureId,
  });
}

export function startSkeletonDrag(
  adapter: PixiInteractionSystem,
  jointIndex: number,
): void {
  const editorState = adapter.editorRef.current;
  ...
  const effectiveBones = getEffectiveBones({
    project,
    effectiveNodes: getEffectiveNodesFromAdapter(adapter),
    editor: editorState,
    animation: adapter.animationRef.current,
  });
  if (
    !effectiveBones?.length ||
    !editorState.skeletonEditMode ||
    jointIndex >= effectiveBones.length
  )
    return;
  ...
  if (!useDraftPose)
    adapter._beginCommandBatch({ name: "Skeleton drag", type: "skeleton" });
  const gestureId =
  ...
  adapter._setDragState({
    type: "skeletonJoint",
    boneId: bone.id,
    startPivotX: bone.setup?.x ?? 0,
    startPivotY: bone.setup?.y ?? 0,
    isAnimMode,
    useDraftPose,
    gestureId,
    setupEffectiveValues: {
      [bone.id]: {
        ...(project.bones.find((candidate) => candidate.id === bone.id)
          ?.setup ?? {}),
      },
    },
  });
  adapter._sendWorkflow({
    type: "START_TRANSFORM_DRAG",
    payload: { mode: "skeletonJoint", boneId: bone.id },
  });
}
