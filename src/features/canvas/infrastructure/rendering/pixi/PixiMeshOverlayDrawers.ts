...
export function drawWarpLattice(
  graphics: Graphics,
  frame: WarpLatticeFrame | null,
  zoom: number,
): void {
  graphics.clear();
  ...
  for (let rowIndex = 0; rowIndex <= row; rowIndex++) {
    for (let columnIndex = 0; columnIndex < col; columnIndex++) {
      ...
      if (first && second)
        graphics.moveTo(first.x, first.y).lineTo(second.x, second.y);
    }
  }
  for (let columnIndex = 0; columnIndex <= col; columnIndex++) {
    for (let rowIndex = 0; rowIndex < row; rowIndex++) {
  ...
  gridPoints.forEach((point, index) => {
    ...
    const isCorner =
      ...
      (columnIndex === 0 || columnIndex === col);
    graphics
      .circle(point.x, point.y, (isCorner ? 6 : 4.5) * invZoom)
      .fill({
        color: isCorner ? 0x50c8ff : 0x32aae6,
        alpha: isCorner ? 0.9 : 0.85,
      });
  });
}
...
export function drawMeshWireframe(
  ...
  for (const vertex of frame.vertices)
    graphics
      .circle(vertex.x, vertex.y, 2.5 * invZoom)
      .fill({ color: 0xffffff, alpha: 0.95 });
}

export function drawIkConstraints(
  graphics: Graphics,
  frame: IkOverlay | null,
  zoom: number,
): void {
  graphics.clear();
  if (!frame) return;
  ...
  for (const target of frame.targets ?? []) {
    const baseRadius = clampIkTargetRadius(
      target.radius ?? IK_TARGET_DEFAULT_RADIUS,
    );
    ...
    const radius = clampIkTargetRadius(
      baseRadius + (target.selected ? 3 : target.hovered ? 1.5 : 0),
    );
    const emphasisRadius = clampIkTargetRadius(radius + 4);
    const color = target.color ?? 0x22d3ee;
    if (target.selected || target.hovered)
      graphics
        .circle(target.x, target.y, emphasisRadius)
        .stroke({
          width: 3 * invZoom,
          color: 0xffffff,
          alpha: target.selected ? 0.9 : 0.55,
        });
    graphics
      ...
      .stroke({
        width: 1.75 * invZoom,
        color,
        alpha: target.assigned ? 0.95 : 0.65,
      });
    graphics
      .moveTo(target.x - radius * 1.5, target.y)
      .lineTo(target.x + radius * 1.5, target.y);
    graphics
      .moveTo(target.x, target.y - radius * 1.5)
      .lineTo(target.x, target.y + radius * 1.5);
...
export function drawWeightPaint(
  graphics: Graphics,
  ...
  if (Array.isArray(frame)) {
    for (const point of frame) {
      const color =
        (Math.round(255 * point.weight) << 16) |
        Math.round(255 * (1 - point.weight));
      graphics
        .circle(point.x, point.y, 3 * invZoom)
  ...
  if (triangles?.length && triangles.length <= 2000)
    for (const triangle of triangles) {
      ...
      if (!first || !second || !third) continue;
      const { color, alpha } = weightToColor(
        ((weights[firstIndex] ?? 0) +
          (weights[secondIndex] ?? 0) +
          (weights[thirdIndex] ?? 0)) /
          3,
      );
      graphics
        .moveTo(first.x, first.y)
