...
function resolveLinkedAnimPatch(
  adapter: PixiInteractionSystem,
  drag: MoveDrag | RotateDrag | ResizeDrag,
  desiredWorldMatrix: Matrix3,
) {
  const project = adapter.projectRef.current;
  const node = project.nodes.find((n) => n.id === drag.nodeId);
  if (!node || !drag.linkedBone || !drag.linkedPreLinkedWorldMatrices)
    return null;
...
function handleResizeDrag(
  ...
  if (drag.linkedAnim) {
    const resolved = resolveLinkedAnimPatch(
      adapter,
      drag,
      scaleAroundWorldPoint(
        ...
        mutator: (project) => {
          if (snapshot) {
            const bone = project.bones.find(
              (candidate) => candidate.id === snapshot.boneId,
            );
            if (bone) Object.assign(bone.setup, snapshot.boneSetup);
            for (const [nodeId, transform] of Object.entries(
              snapshot.nodeTransforms,
            )) {
...
function handleMoveDrag(
  ...
  if (drag.linkedAnim) {
    ...
    const boneDelta = mat3Mul(
      makeLocalMatrix(boneOverrides),
      mat3Inverse(makeLocalMatrix(boneSetup)),
    );
    ...
    adapter._executeCommand({
      ...
      payload: {
        mutator: (project) =>
          translateLinkedNodeGroup(project, drag.nodeId, stepDx, stepDy),
      },
    });
  } else {
...
function handleRotateDrag(
  ...
  if (drag.linkedAnim) {
    ...
    const boneDeltaRotation =
      Math.atan2(boneDelta[1], boneDelta[0]) * (180 / Math.PI);
    const srcRotation = drag.startRotation - boneDeltaRotation;
    const resolved = resolveLinkedAnimPatch(
      adapter,
      drag,
      (() => {
        const nodeLocal = makeLocalMatrix({
    ...
    adapter._executeCommand({
      ...
      payload: {
        mutator: (project) =>
          rotateLinkedNodeGroup(project, drag.nodeId, stepDelta),
      },
    });
  } else {
...
function handlePivotDrag(
  ...
  const projectNode = adapter.projectRef.current.nodes.find(
    (n) => n.id === drag.nodeId,
  );
...
function handleBoneMoveDrag(
  ...
  adapter._executeCommand({
    type: "updateProject",
    // Setup edits move bind data only; frame evaluation applies bind→IK-pose deltas
    // to linked nodes, so resolved IK here would transform the image twice.
    payload: {
      mutator: (project) =>
        translateLinkedBoneSelection(
          project,
...
function handleBoneRotateDrag(
  ...
  if (drag.startAngle === null) {
    drag.startAngle = Math.atan2(
      worldPos.y - drag.pivotY,
      worldPos.x - drag.pivotX,
    );
    return true;
  }
  ...
  if (drag.useDraftPose) {
    ...
    for (const boneId of boneIds) {
      const start = drag.startBones?.[boneId] ?? {};
      const partial: { rotation: number; x?: number; y?: number } = {
        rotation: (start.rotation ?? 0) + delta,
      };
      if (boneIds.length > 1) {
  ...
  if (drag.useDraftPose) {
    for (const boneId of drag.boneIds ?? [drag.boneId]) {
      previewPosePartial(adapter, boneId, {
        length: Math.max(
          10,
          (drag.startLengths?.[boneId] ?? drag.startLength) * factor,
        ),
      });
    }
    adapter.markDirty();
...
function ensureSetupPoseCleared(
  adapter: PixiInteractionSystem,
  drag: BoneMoveDrag | BoneRotateDrag | BoneLengthDrag,
): void {
