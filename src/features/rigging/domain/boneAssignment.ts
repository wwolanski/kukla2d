...
export function isNodeAssignedToBone(
  node: AssignableNode | null | undefined,
  ...
  if (
    ...
    return true;
  return (
    node.mesh?.influences?.some((vertex) =>
      vertex.some((inf) => inf.boneId === bone.id),
    ) ?? false
  );
}
 ...
 */
export function isNodeDirectlyAssignedToBone(
  node: AssignableNode | null | undefined,
  bone: Bone | null | undefined,
): boolean {
  if (!node || !bone) return false;
...
export function doesBoneInfluenceNode(
  node: AssignableNode | null | undefined,
  boneId: BoneId | null | undefined,
): boolean {
  if (!node?.mesh || !boneId) return false;
  return (
    node.mesh.influences?.some((vertex) =>
      vertex?.some(
        (influence) => influence?.boneId === boneId && influence.weight > 0,
      ),
    ) ?? false
  );
}

function uniqueBoneIds(
 ...
 */
export function getNodeMeshInfluenceBoneIds(
  node: AssignableNode | null | undefined,
): string[] {
  if (!node) return [];
  ...
  const weighted =
    node.mesh?.influences?.flatMap((vertex) =>
      (vertex ?? [])
        .filter((influence) => influence?.weight > 0)
        .map((influence) => influence.boneId),
    ) ?? [];
  return uniqueBoneIds([...weighted, node.boneId, node.mesh?.jointBoneId]);
}

export function setNodeMeshInfluenceBone(
  node: AssignableNode | null | undefined,
  boneId: BoneId | null | undefined,
  included: boolean,
): void {
 ...
 */
export function assignNodeToBone(
  node: AssignableNode | null | undefined,
  boneId: BoneId | null | undefined,
): void {
  if (!node || !boneId) return;
  ...
  if (!hasWeights) {
    node.mesh.influences = Array.from({ length: count }, () => [
      { boneId, weight: 1 },
    ]);
 ...
 */
export function clearNodeBoneAssignment(
  node: AssignableNode | null | undefined,
): void {
  if (!node) return;
  node.boneId = null;
...
export function assignProjectNodeToBone(
 ...
 */
export function assignOrAddProjectNodeBoneInfluence(
  project: RigProject | null | undefined,
  nodeId: NodeId | null | undefined,
  boneId: BoneId | null | undefined,
): BoneAssignmentAction {
  ...
  const legacyOwner =
    (project.bones ?? []).find((bone) => bone.nodeId === nodeId)?.id ?? null;
...
export function clearProjectNodeBoneAssignment(
  project: RigProject | null | undefined,
  nodeId: NodeId | null | undefined,
): void {
 ...
 */
export function isBoneLinkLocked(
  node: AssignableNode | null | undefined,
): boolean {
 ...
 */
export function setBoneLinkLocked(
  node: AssignableNode | null | undefined,
  locked: boolean,
): void {
  if (!node) return;
...
export function getAssignedBoneForNode(
  project: RigProject | null | undefined,
  nodeId: NodeId | null | undefined,
): Bone | null {
...
export function isLinkedNodeAssignedToBone(
  node: AssignableNode | null | undefined,
  bone: Bone | null | undefined,
): boolean {
...
export function getLinkedNodesForBone(
  project: RigProject | null | undefined,
  boneId: BoneId | null | undefined,
): Node[] {
  ...
  return (project.nodes ?? []).filter((node) =>
    isLinkedNodeAssignedToBone(node, bone),
  );
}
