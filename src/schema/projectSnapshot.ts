...
type PortablePartNode = Omit<Extract<Node, { type: "part" }>, "mesh"> & {
  mesh?: PortableMesh | null;
};

type PortableNode =
  PortablePartNode | Extract<Node, { type: "group" | "warpDeformer" }>;
...
function deepCloneJsonSafe(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error(`Non-finite number "${String(value)}" is not JSON-safe`);
    return value;
  }
  ...
  if (value instanceof Map) {
    return Object.fromEntries(
      Array.from(value, ([key, entryValue]) => [
        String(key),
        deepCloneJsonSafe(entryValue),
      ]),
    );
  }
...
function normalizeMeshForPortable(
  mesh: Mesh | null | undefined,
): PortableMesh | null | undefined {
  if (!mesh) return mesh;
  return {
    ...mesh,
    uvs: Array.isArray(mesh.uvs) ? [...mesh.uvs] : Array.from(mesh.uvs),
    edgeIndices: Array.isArray(mesh.edgeIndices)
      ? [...mesh.edgeIndices]
      : Array.from(mesh.edgeIndices),
    ...(mesh.boneWeights
      ? {
          boneWeights: Array.isArray(mesh.boneWeights)
            ? [...mesh.boneWeights]
            : Array.from(mesh.boneWeights),
        }
      : {}),
  };
}
  ...
  if (Array.isArray(snapshot.nodes)) {
    snapshot.nodes = snapshot.nodes.map((node) => {
      if (node.type === "part" && node.mesh) {
        return {
          ...node,
          mesh: normalizeMeshForPortable(node.mesh),
        } as PortableNode;
      }
      return node;
    });
...
function assertPortableProjectDocument(
  value: unknown,
): asserts value is PortableProjectDocument {
  if (
    !value ||
    typeof value !== "object" ||
  ...
  for (const node of (value as { nodes: unknown[] }).nodes) {
    if (!node || typeof node !== "object")
      throw new Error("Portable snapshot contains an invalid node");
...
function assertJsonSafeRecursive(
  value: unknown,
  path: string,
  ...
  if (value === undefined)
    throw new Error(`undefined at ${path} is not JSON-safe`);
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error(`Non-finite number at ${path} is not JSON-safe`);
    return;
  }
