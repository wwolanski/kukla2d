...
function inferLegacyCanvasPresetId(canvas: {
  width: number;
  height: number;
}): string {
  return (
    LEGACY_CANVAS_PRESETS.find(
...
export function pickPersistedProjectFields(
  project: ProjectDocument,
): PersistedProjectFields;
export function pickPersistedProjectFields(
  project: unknown,
): PersistedProjectFields {
...
export function prepareLoadedProjectDocument(
  ...
  project.version = Number.isFinite(numericVersion)
    ? numericVersion
    : defaults.version;
  project.author =
    typeof incoming.author === "string" ? incoming.author : defaults.author;
  project.lastActiveAnimationId =
    incoming.lastActiveAnimationId ?? defaults.lastActiveAnimationId;
  ...
  if (
    !Object.prototype.hasOwnProperty.call(incoming.canvas ?? {}, "presetId")
  ) {
    project.canvas.presetId = inferLegacyCanvasPresetId(
      project.canvas,
    ) as Exclude<Canvas["presetId"], undefined>;
  }
  project.textures = deepClone(
    incoming.textures ?? defaults.textures,
  ) as typeof defaults.textures;
  project.bones = deepClone(
    incoming.bones ?? defaults.bones,
  ) as typeof defaults.bones;
  project.slots = deepClone(
    incoming.slots ?? defaults.slots,
  ) as typeof defaults.slots;
  project.attachments = deepClone(
    incoming.attachments ?? defaults.attachments,
  ) as typeof defaults.attachments;
  project.skins = deepClone(
    incoming.skins ?? defaults.skins,
  ) as typeof defaults.skins;
  project.constraints = deepClone(
    incoming.constraints ?? defaults.constraints,
  ) as typeof defaults.constraints;
  project.defaultPose = deepClone(
    incoming.defaultPose ?? defaults.defaultPose,
  ) as typeof defaults.defaultPose;
  ...
  project.nodes = nodes;
  project.animations = normalizeAnimations(
    deepClone(incoming.animations ?? defaults.animations),
  );

  project.physics_groups = deepClone(
    incoming.physics_groups ?? defaults.physics_groups,
  ) as typeof defaults.physics_groups;
  project.physicsRules = deepClone(
    incoming.physicsRules ?? defaults.physicsRules,
  ) as typeof defaults.physicsRules;
  project.libraryFolders = deepClone(
    incoming.libraryFolders ?? defaults.libraryFolders,
  ) as typeof defaults.libraryFolders;
  project.assetPlacements = deepClone(
    incoming.assetPlacements ?? defaults.assetPlacements,
  ) as typeof defaults.assetPlacements;
  project.modularSprites = deepClone(
    incoming.modularSprites ?? defaults.modularSprites,
  ) as typeof defaults.modularSprites;
  project.controlHandles = deepClone(
    incoming.controlHandles ?? defaults.controlHandles,
  ) as typeof defaults.controlHandles;
  const modifiers = deepClone(
    incoming.animationModifiers ?? defaults.animationModifiers,
  ) as typeof defaults.animationModifiers;
