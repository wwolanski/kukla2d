...
function isProjectArchiveManifest(
  value: unknown,
): value is ProjectArchiveManifest {
...
function audioExtension(mimeType: string | null | undefined): string {
  ...
  return (
    subtype.replace(/[^a-z0-9]+/gi, "").toLowerCase() || DEFAULT_AUDIO_EXTENSION
  );
}

async function fetchBlobOrThrow(
  url: string,
  assetId: string,
...
function zipFileOrThrow(
  ...
  if (!entry) {
    throw new AssetResolveError(
      assetId,
      assetType,
      new Error(`missing archive entry "${path}"`),
    );
  }
  return entry;
}
...
export async function saveProject(project: ProjectDocument): Promise<Blob> {
  ...
  for (const tex of project.textures ?? []) {
    if (!tex.source) {
      errors.push(
        new AssetResolveError(tex.id, "texture", new Error("missing source")),
      );
    }
  }
      ...
      if (track.source && !track.sourceUrl) {
        errors.push(
          new AssetResolveError(
            track.id,
            "audio",
            new Error("missing runtime source"),
          ),
        );
      }
    }
  ...
  if (errors.length > 0) {
    const err = new Error(
      `Cannot save: ${errors.length} asset error(s)`,
    ) as Error & { errors: AssetResolveError[] };
...
async function inspectZip(
  ...
  try {
    zip = await new (
      JSZipCtor as new () => { loadAsync(data: unknown): Promise<ZipReader> }
    )().loadAsync(file);
  } catch (cause: unknown) {
    throw new Error(
      `Invalid .kk2d project file: cannot read ZIP (${isRecord(cause) && typeof cause.message === "string" ? cause.message : String(cause)})`,
      { cause: cause instanceof Error ? cause : undefined },
    );
  }
  ...
  if (manifestFile) {
    ...
    try {
      ...
      throw makeErrorWithCause(
        `Invalid .kk2d project file: invalid manifest (${isRecord(cause) && typeof cause.message === "string" ? cause.message : String(cause)})`,
        cause,
      );
    }
    ...
    if (manifest.formatId !== PROJECT_ARCHIVE_FORMAT_ID) {
      throw new Error(
        `Invalid .kk2d project file: unexpected formatId "${String(manifest.formatId)}"`,
      );
    }
  }
  ...
  if (entries.length > MAX_ENTRIES) {
    throw new Error(
      `ZIP contains ${entries.length} entries, max allowed is ${MAX_ENTRIES}`,
    );
  }
  ...
  if (totalSize > MAX_TOTAL_SIZE) {
    throw new Error(
      `ZIP total size ${totalSize} exceeds limit ${MAX_TOTAL_SIZE}`,
    );
  }
...
async function readProjectJson(zip: ZipReader): Promise<unknown> {
  ...
  try {
    ...
    throw makeErrorWithCause(
      `Invalid .kk2d project file: invalid project.json (${isRecord(cause) && typeof cause.message === "string" ? cause.message : String(cause)})`,
      cause,
    );
  }
}
...
async function materializeBlobUrls(
  ...
  for (const tex of project.textures) {
    ...
    const pngBlob = await zipFileOrThrow(
      zip,
      tex.source,
      tex.id,
      "texture",
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      img.onerror = () =>
        reject(new Error(`Failed to decode texture ${tex.id}`));
      img.src = blobUrl;
    });
  ...
  for (const animation of project.animations ?? []) {
    ...
    for (const track of animation.audioTracks) {
      if (track.source) {
        const audioBlob = await zipFileOrThrow(
          zip,
          track.source,
          track.id,
          "audio",
