...
interface ProjectRecordMetadata {
  formatId?: string;
...
export function isStoredProjectRecord(
  value: unknown,
): value is StoredProjectRecord {
  ...
  if (typeof value.formatId !== "string" || value.formatId.length === 0)
    return false;
...
export function isValidRecoveryRecord(value: unknown): value is RecoveryRecord {
  ...
  if (!Number.isFinite(value.savedAt) || (value.savedAt as number) <= 0)
    return false;
  if (
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      if (!Array.isArray(raw)) {
        return reject(
          new Error("IndexedDB returned non-array result for listProjects"),
        );
      }
      ...
      for (const entry of raw) {
        if (isStoredProjectRecord(entry)) {
          ...
          return reject(
            new Error(
              `Invalid project record in IndexedDB store "${STORE_NAME}" (id: ${id})`,
            ),
          );
        }
      }
...
export async function updateProjectName(
  ...
  return new Promise((resolve, reject) => {
    ...
    getRequest.onerror = () =>
      reject(
        getRequest.error ?? new Error("Failed to read project for rename"),
      );
  });
}
...
export async function updateProjectAuthor(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve();
    request.onerror = () =>
      reject(request.error ?? new Error("Failed to update project author"));
  });
}
...
export async function duplicateProject(id: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    getRequest.onerror = () =>
      reject(
        getRequest.error ?? new Error("Failed to read project for duplicate"),
      );
  });
}
...
export async function readRecovery(): Promise<RecoveryRecord | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("Failed to read recovery record"));
  });
}
...
export async function clearRecovery(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.onerror = () =>
      reject(
        transaction.error ?? new Error("Recovery clear transaction failed"),
      );
    transaction.onabort = () =>
      reject(
        transaction.error ?? new Error("Recovery clear transaction aborted"),
      );
  });
}
