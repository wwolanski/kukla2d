...
} from "../contracts/semantic.types.js";
...
export const BUILTIN_SEMANTIC_DEFINITIONS: readonly SemanticDefinition[] = [
  ...definitions("part-role", roleKeys),
  ...definitions("character-type", typeKeys),
  ...definitions("character-class", classKeys),
];

export class SemanticCatalog {
  readonly #definitions = new Map<string, SemanticDefinition>();
  constructor(
    initial: readonly SemanticDefinition[] = BUILTIN_SEMANTIC_DEFINITIONS,
  ) {
    for (const item of initial) this.upsert(item);
  }
  list(kind?: SemanticDefinitionKind): SemanticDefinition[] {
    return [...this.#definitions.values()]
      .filter((item) => !kind || item.kind === kind)
      .sort((a, b) => a.label.localeCompare(b.label));
  }
  get(id: string): SemanticDefinition | undefined {
    return this.#definitions.get(id);
  }
  find(
    kind: SemanticDefinitionKind,
    value: string,
  ): SemanticDefinition | undefined {
  ...
  upsert(definition: SemanticDefinition): void {
    if (!definition.id || !definition.key || definition.revision < 1)
      throw new Error("Invalid semantic definition");
...
export const semanticRoleIdForLegacyRole = (role: string): string | undefined =>
  BUILTIN_SEMANTIC_DEFINITIONS.find(
    (item) => item.kind === "part-role" && item.key === role,
  )?.id;
