...
export class BrowserTaskService implements TaskService {
  ...
  dispatch<TPayload, TData>(
    ...
    if (this.state === "disposed") {
      return Promise.reject(
        new BrowserTaskServiceError("disposed", "Task service is disposed"),
      );
    }
    if (!isValidRequest(request)) {
      return Promise.reject(
        new BrowserTaskServiceError("invalid-message", "Invalid task request"),
      );
    }
    if (this.tasks.has(request.requestId)) {
      return Promise.reject(
        new BrowserTaskServiceError(
          "invalid-message",
    ...
    try {
      ...
      return Promise.reject(
        error instanceof BrowserTaskServiceError
          ? error
          : new BrowserTaskServiceError(
              "worker-error",
    ...
    return new Promise<TaskResult<unknown>>((resolve, reject) => {
      ...
      const handler = (event: MessageEvent<unknown>): void => {
        ...
        if (message.data.projectRevision !== request.projectRevision) {
          reject(
            new BrowserTaskServiceError("stale-revision", "Stale revision"),
          );
          return;
        }
        try {
          const data = decodeResult
            ? decodeResult(message.data.data)
            : message.data.data;
          resolve({
            requestId: message.data.requestId,
            projectRevision: message.data.projectRevision,
            data,
          });
        } catch (error: unknown) {
          reject(
  ...
  dispose(): void {
    ...
    for (const [requestId, task] of this.tasks) {
      task.worker.removeEventListener("message", task.handler);
      task.reject(
        new BrowserTaskServiceError("disposed", "Task service disposed"),
      );
      this.tasks.delete(requestId);
    }
...
export function parseTaskMessage(value: unknown): TaskMessageParseResult {
  ...
  if (typeof requestId !== "string")
    return invalidMessage("Worker requestId is invalid");
  if (value.type === "progress") {
    if (
      !isFiniteNumber(data.progress) ||
      data.progress < 0 ||
      data.progress > 1 ||
  ...
  if (value.type === "result") {
    if (!isRevision(data.projectRevision) || !("data" in data))
      return invalidMessage("Worker result message is invalid");
    return {
      ok: true,
      message: {
        type: "result",
...
function createDefaultWorker(kind: TaskKind): Worker {
  if (kind !== "mesh.generate")
    throw new BrowserTaskServiceError(
      "invalid-message",
      `Unsupported browser task kind: ${kind}`,
    );
  return new Worker(
    new URL(
      "../../../src/features/canvas/infrastructure/mesh-worker/worker.ts",
      import.meta.url,
    ),
    { type: "module" },
  );
...
function isValidRequest<TPayload>(request: TaskRequest<TPayload>): boolean {
  return (
    ...
    isRevision(request.projectRevision) &&
    ["mesh.generate", "psd.parse", "inference.pose", "export.frames"].includes(
      request.kind,
    )
  );
}
